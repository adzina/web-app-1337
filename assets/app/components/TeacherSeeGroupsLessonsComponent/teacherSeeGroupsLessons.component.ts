...
export class TeacherSeeGroupsLessonsComponent {
  ...
  constructor(
    ...
    _backendService.getGroupsLessons(this.group.id).subscribe(
      response => {
  ...
  getData() {
    this._backendService.getActiveUsers(this.group.id).subscribe(data => {
      for (let user of data) {
        ...
        this._backendService
          .countWordsForManyStudents(ids, this.group.id, 1)
          .subscribe(data => {
            ...
            for (let record of sortedData) {
              ...
              } else { this.progress.push(0); }
            }
            this._backendService
              .countWordsForManyStudents(ids, this.group.id, 4)
              .subscribe(data1 => {
